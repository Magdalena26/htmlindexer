...
{		
	...
	public List<Sentence> findSentencesByWords(String words)
	{
		...
		List<Sentence> result = session.createQuery("from Sentence s where s.content like :query", Sentence.class).setParameter("query", query).getResultList();
	...
	public List<String> findSentenceByLength(int length) {
		...
		List<String> result = session.createQuery("select s.content from Sentence s where LENGTH(s.content) > :length", String.class).setParameter("length", length).getResultList();
